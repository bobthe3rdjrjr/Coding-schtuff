# What is this repo?
Hey! These are my personal coding projects, some are from roadmap.sh, some are stuff I found online, & some are just whatever. The individual folders with my projects will have their own readme's in them with actual instructions so just check stuff out! Here are what all the projects I have do and other resources:

* **Unit Converter:** My first ever project on this repo, made from a roadmap.sh project (https://roadmap.sh/projects/unit-converter), simply converts length, weight, & temperature units, uses:
    * Python for the backend
    * Html for the frontend
    * Flask to link the two
    * css for dark mode

* **Todo-List-API:** Second project, a simple api for a todo list which is also from [roadmap.sh](https://roadmap.sh/projects/todo-list-api), it uses:
   * Python for like everything
   * Flask to connect
   * Uh thats all
 
* **tmdb-cli tool** A simple project from [roadmap.sh](https://roadmap.sh/projects/tmdb-cli) that uses request to call the tmdb [api](https://www.themoviedb.org/), uses:
   * Python
   * Requests (from pip)
